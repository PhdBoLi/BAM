...
seq=['tb_phase_ch','tb_switch_en_train','tb_switch_en_test','tb_switch_en','tb_arb_clk_x','tb_arb_clk_y','tb_jk_clk','tb_jk_train_clk','tb_jk_test_clk','tb_lu_src_clk','tb_lu_clk','tb_sampling_y','tb_amp_n','tb_amp_n_sx','tb_amp_n_sy','tb_amp_out','tb_amp_out_sx','tb_amp_out_sy','tb_s_weight_xbias','tb_s_weight_ybias']
#seq=['tb_phase_ch','tb_switch_en']
for i in seq:
...
for i in range(1,xnum+1):
  ...
  f.writelines(['probe -create -emptyok -database ams_database {Top_Recall_Only.tb_x_out[',i,']}\n'])
...
for j in range(1,ynum+1):
  ...
  f.writelines(['probe -create -emptyok -database ams_database {Top_Recall_Only.tb_y_out_res[',j,']}\n'])
